...
def parse_msf_exploit_samba_usermap_script(stdout: str, stderr: str, target_ip: str | None, target_port: int | None) -> list[dict]:
    return parse_msfconsole_exploit(stdout, stderr, target_ip, target_port, exploit_name="samba_usermap_script", service="smb")


def parse_msf_exploit_vsftpd_234_backdoor(stdout: str, stderr: str, target_ip: str | None, target_port: int | None) -> list[dict]:
    return parse_msfconsole_exploit(stdout, stderr, target_ip, target_port, exploit_name="vsftpd_234_backdoor", service="ftp")


def parse_msf_exploit_distcc_exec(stdout: str, stderr: str, target_ip: str | None, target_port: int | None) -> list[dict]:
    return parse_msfconsole_exploit(stdout, stderr, target_ip, target_port, exploit_name="distcc_exec", service="distcc")

def parse_msf_exploit_postgres_payload(stdout: str, stderr: str, target_ip: str | None, target_port: int | None) -> list[dict]:
    return parse_msfconsole_exploit(stdout, stderr, target_ip, target_port, exploit_name="postgres_payload", service="postgresql")


def parse_msf_exploit_unreal_ircd_backdoor(stdout: str, stderr: str, target_ip: str | None, target_port: int | None) -> list[dict]:
    return parse_msfconsole_exploit(stdout, stderr, target_ip, target_port, exploit_name="unreal_ircd_3281_backdoor", service="irc",
    )

def parse_msf_exploit_tomcat_mgr_upload(stdout: str, stderr: str, target_ip: str | None, target_port: int | None) -> list[dict]:
    return parse_msfconsole_exploit(stdout, stderr, target_ip, target_port, exploit_name="tomcat_mgr_upload")



def parse_msfconsole_exploit(stdout: str, stderr: str, target_ip: str | None, target_port: int | None, exploit_name: str) -> list[dict]:
    events = [{
    ...
    patterns = [
        (r"Command shell session (?P<session_id>\d+) opened", "shell"),
        (r"Meterpreter session (?P<session_id>\d+) opened", "meterpreter"),
        (r"SSH session (?P<session_id>\d+) opened", "ssh"),
        (r"session (?P<session_id>\d+) opened", "shell"),
    ]
    ...
    if session_id:
        ...
        if "session 1 closed" in lower or "command shell session 1 closed" in lower or "closed" in lower or "killing all sessions" in lower:
            events.append({
                "type": "SESSION_CLOSED",
                "host": target_ip,
                "port": target_port,
